...
{
    return entry & 0x0f;    // Return the last 4 bits
}
...
int heap_get_start_block(struct heap* heap, uint32_t total_blocks)
{
    ...
    int currentBlock = 0;
    int startingBlock = -1;     // the first free block found
...
{
    return heap->startAddress + (block * PEACHOS_HEAP_BLOCK_SIZE);
}
...
{
    return ((int)(address - heap->startAddress)) / PEACHOS_HEAP_BLOCK_SIZE;
}
